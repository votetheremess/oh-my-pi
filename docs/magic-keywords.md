...
| `ultracode`   | Runs that turn at `xhigh` reasoning effort, for the turn and every subagent it spawns (overriding an agent's own pinned effort), and carries the full `workflowz` orchestration contract so the turn runs as a multi-subagent workflow. The effort is borrowed for the turn and handed back afterwards; nothing is persisted to disk.                                     |
...
- The hidden `ultracode-notice` carries the same orchestration contract `workflowz` injects: the `eval` helper signatures, the workflow structure, and the fan-out patterns, embedded in the notice rather than referred to. Naming the contract without carrying it would tell the model to orchestrate while withholding the API it must orchestrate with.
