...
- The hidden `ultracode-notice` is re-injected on every user turn, and it carries the same orchestration contract `workflowz` injects: the `eval` helper signatures, the workflow structure, and the fan-out patterns, embedded in the ultracode notice rather than referred to. What ultracode adds on top is that the contract is standing. A workflow is the default shape for every substantive task for the rest of the session, not just the turn that carried the keyword, and the notice says so in as many words.
- That contract is gated on tool availability, because there is no way to fan out without it. When `eval` and `task` are both active the full API ships. When either is missing the notice is still injected, unlike `workflowz` which is skipped outright, because the effort pin applies regardless; it drops the fan-out API and states plainly that orchestration is unavailable, so the model does not describe subagents it cannot run.
- Synthetic, agent-initiated turns never re-inject it; the pinned effort simply stays where the last user turn set it.
