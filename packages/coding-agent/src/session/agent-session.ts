...
export class AgentSession {
	 ...
	 * joins, so a keyword-free steer attaches no hook at all.
	 *
	 ...
	 */
	#ultracodeDeliveryHook(text: string, mode: "steer" | "followUp"): (() => void) | undefined {
		if (this.#agentKind === "sub") return undefined;
		if (this.#magicKeywordEnabled("ultracode") && containsUltracode(text)) {
			return () => {
		...
		if (mode === "steer") return undefined;
	...
	async prompt(text: string, options?: PromptOptions): Promise<boolean> {
		...
		if (this.isStreaming) {
			...
			// queue mode shapes it: a steer joins the running turn and is arm-only.
	...
	async sendUserMessage(
		...
		// delivery hooks as steer()/followUp() (a steer is arm-only, a follow-up
		// starts a turn), and the direct one keeps user attribution so keyword
		// notices, the TUI turn-time anchor, and warp's prompt_submit see an
		// extension-initiated turn as the user turn it is. Root-only gating in
